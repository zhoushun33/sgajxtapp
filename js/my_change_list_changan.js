...
mui.plusReady(function(){
	...
	mui('#caogao').on('longtap', 'ul', function() {
		...
		//总公司签到判断
	var qdpd=function(info){
//		alert(info);
			var ulId=info[0];
			var gcid = info[1];
			var sjc = info[2];
			var mobile = info[3];
//			alert(gcid+sjc+mobile);
		mui.ajax(url+'my_plus/zgxf_qdyz.php',{
			data:{
				gcid : gcid,
				sjc : sjc,
				mobile : mobile
...
//				alert(data.identity+'|'+data.result);
				if(data.identity=='gs'){
					if(data.result=='ok'){
						xfzg(ulId,gcid,gcmc);
					}else if(data.result=='no'||data.result==" "){
						alert("请先签到再下发整改！")
					}
					}else if(data.identity=='xmb'){
						xfzg(ulId,gcid,gcmc);
					}
			},
...
//					break;
	...
	//删除草稿
	var sccg=function xfzg(ulinfo,gcid,gcmc){
//		alert(ulId);
		var caid=ulinfo;
		var gcid=gcid;
		var gcmc=gcmc;
